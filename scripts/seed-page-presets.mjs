...
async function run() {
  ...
  const response = await fetch(`${baseUrl}/api/internal/seed-page-presets`, {
    method: 'POST',
    headers: {
  ...
  console.log(
    `Template seed complete: created=${payload.created}, hydrated=${payload.hydrated}, existing=${payload.existing}`,
  );
  if (Array.isArray(payload.items)) {
    for (const item of payload.items) {
      console.log(`- ${item.presetKey} (${item.name}): ${item.action} (${item.id})`);
    }
  }
}
