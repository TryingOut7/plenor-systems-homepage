...
async function run() {
  ...
  const response = await fetch(`${baseUrl}/api/internal/seed-site-pages`, {
    method: 'POST',
    headers: {
  ...
  console.log(`Seed complete: created=${payload.created}, existing=${payload.existing}`);
  if (Array.isArray(payload.items)) {
    for (const item of payload.items) {
      console.log(`- ${item.slug}: ${item.action} (${item.id})`);
    }
  }
}
