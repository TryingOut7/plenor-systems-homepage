...
import { createPresetFromDraft } from '@/application/workspaces/workspaceMutationService';
...
export async function POST(
  ...
  const proxied = await proxyRequestToBackend(
    request,
    `/v1/pages/drafts/${resolvedParams.id}/create-preset`,
  );
  if (proxied) {
    return proxied;
  }
  ...
  try {
    ...
    const preset = await createPresetFromDraft(repository, {
