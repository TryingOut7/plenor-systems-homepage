...
import { createPresetFromPlayground } from '@/application/workspaces/workspaceMutationService';
...
export async function POST(
  ...
  const proxied = await proxyRequestToBackend(
    request,
    `/v1/pages/playgrounds/${resolvedParams.id}/create-preset`,
  );
  if (proxied) {
    return proxied;
  }
  ...
  try {
    ...
    const preset = await createPresetFromPlayground(repository, {
    ...
    return toApiErrorResponse(error, 'Failed to create preset.');
  }
}
