...
export default function FormRenderer({ formId, successMessage, theme }: FormRendererProps) {
  ...
  if (loading) {
    return <p style={{ color: dark ? 'var(--ui-color-dark-text-muted)' : 'var(--ui-color-text-muted)', fontSize: '14px' }}>Loading form...</p>;
  }
  ...
  if (submitted) {
    const msg = successMessage || 'Thank you! Your submission has been received.';
    return (
      <div
        style={{
          padding: '32px',
      ...
      >
        <p style={{ fontSize: '16px', fontWeight: 600, color: dark ? 'var(--ui-color-dark-text)' : 'var(--ui-color-primary)', margin: 0 }}>
          {msg}
        </p>
      </div>
    );
          ...
          <div key={field.id || field.name}>
            {field.fieldType === 'checkbox' ? (
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', cursor: 'pointer' }}>
                ...
                />
                <span style={{ fontSize: '14px', color: dark ? 'var(--ui-color-dark-text)' : 'var(--ui-color-primary)' }}>
                  {field.label}
                  {field.required ? <span aria-hidden="true" style={{ color: '#dc2626', marginLeft: '4px' }}>*</span> : null}
                </span>
              </label>
