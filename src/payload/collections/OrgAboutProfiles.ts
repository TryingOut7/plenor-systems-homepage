...
export const OrgAboutProfiles: CollectionConfig = {
  slug: 'org-about-profiles',
  admin: {
    useAsTitle: 'name',
    defaultColumns: ['name', 'slug', 'category', 'displayOrder', 'workflowStatus'],
    group: 'Org Site',
    description: 'About profiles: founders, volunteer team members, and mentors.',
  },
  access: {
    read: ({ req }) => {
  ...
  fields: [
    ...
    },
    {
      ...
      type: 'select',
      required: true,
      options: toPayloadOptions(ABOUT_CATEGORIES),
    ...
    // ─── Content ───────────────────────────────────────────────────────
    {
      ...
      maxLength: 320,
      admin: {
        description: 'Short biography for cards and search snippets (max 320 chars).',
      },
    },
    {
      name: 'detailContent',
    ...
    },
    {
      ...
      required: true,
      admin: {
        description: 'Profile image (recommended 400×400, 1:1).',
      },
    },
    ...
    // ─── Optional Details ──────────────────────────────────────────────
    {
      name: 'roleTitle',
      type: 'text',
      admin: { description: 'Role or title (e.g. "Co-Founder", "Lead Mentor").' },
    },
    {
      name: 'affiliation',
      type: 'text',
      admin: { description: 'Organization or other affiliation.' },
    },
    ...
    // ─── Media & Links ─────────────────────────────────────────────────
    {
      ...
      type: 'array',
      admin: { description: 'Additional images for this profile.' },
      fields: [
        {
          name: 'image',
          type: 'upload',
    ...
    },
    {
      ...
      admin: { description: 'Optional external link (e.g. personal website, LinkedIn).' },
    },
